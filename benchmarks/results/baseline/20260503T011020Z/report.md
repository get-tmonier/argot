...
Generated: 2026-05-03T01:23:23.478297+00:00
...
| rich | python | 0.9780 | 100.0% | 1.2% | -3.040 | 16 | 68598 | 3.842 |
| faker | python | 0.9537 | 95.0% | 2.0% | -4.946 | 16 | 75996 | 5.257 |
| hono | typescript | 0.8312 | 88.3% | 0.5% | -7.471 | 17 | 54717 | 4.289 |
| ink | typescript | 0.9899 | 93.3% | 0.5% | -4.633 | 17 | 16678 | 4.993 |
| faker-js | typescript | 0.9463 | 71.7% | 0.9% | -7.066 | 17 | 255760 | 4.861 |
...
- **FP rate on real PR hunks:** 0.6% (176/79623)
...
Threshold **5.2585** — 9/32 breaks fall below it (misses), 91/79623 controls fall at/above (false positives).
...
- **exception_handling_4** (`exception_handling`) — score 3.0281, 2.2304 below threshold, reason: `none`
...
- **validation_2** (`validation`) — score 2.9797, 2.2788 below threshold, reason: `none`
...
- **routing_3** (`routing`) — score 2.9797, 2.2788 below threshold, reason: `none`
...
Threshold **3.8424** — 3/16 breaks fall below it (misses), 101/68598 controls fall at/above (false positives).
...
| print_manual | 100.0% | 4/4 | 3.795 | 1.607 | 6.730 | print_manual_1, print_manual_2, print_manual_3, dict_render_1 |
...
| dict_render_1 | print_manual | 1.607 | ✓ | call_receiver | breaks/break_dict_render_1.py | 18–23 | plain print() loop in place of rich Table/Panel; no foreign import in hunk; era-5 limit: print/for/list tokens are ubiquitous in Python c… |
...
_Threshold is 3.8424; top control scores 4.6469._
...
- **FP rate on real PR hunks:** 2.0% (663/75996)
...
Threshold **5.2572** — 6/16 breaks fall below it (misses), 163/75996 controls fall at/above (false positives).
...
| numpy_random_3 | numpy_random | 5.304 | ✓ | bpe | breaks/break_numpy_random_3.py | 15–32 | numpy rng.choice and rng.shuffle called on a default_rng instance — receiver methods foreign to faker corpus. rng.integers is known but r… |
...
| sqlalchemy_sink_3 | sqlalchemy_sink | 5.109 | ✓ | bpe | breaks/break_sqlalchemy_sink_3.py | 15–30 | SQLAlchemy Session.bulk_save_objects + Session.flush + Session.expire_all — these receiver methods on a Session object are foreign to fak… |
...
Breaks that didn't trip the scorer (threshold 5.2572):

- **synthetic_formula_1** (`numpy_random`) — score 3.8133, 1.4439 below threshold, reason: `none`
...
_Threshold is 5.2572; top control scores 6.7890._
...
- **FP rate on real PR hunks:** 0.5% (164/54717)
...
Threshold **4.2891** — 7/17 breaks fall below it (misses), 153/54717 controls fall at/above (false positives).
...
| framework_swap | 100.0% | 4/4 | 4.053 | 1.484 | 6.373 | hono_framework_swap_1, hono_framework_swap_2, hono_framework_swap_3, hono_framework_swap_4 |
...
- **hono_validation_2** (`validation`) — score 2.2307, 2.0583 below threshold, reason: `none`
...
_Threshold is 4.2891; top control scores 5.7353._
...
- **FP rate on real PR hunks:** 0.5% (39/16678)
...
Threshold **4.9932** — 2/17 breaks fall below it (misses), 27/16678 controls fall at/above (false positives).
...
_Threshold is 4.9932; top control scores 6.7376._
...
- **Recall (mean across categories):** 71.7%
- **FP rate on real PR hunks:** 0.9% (410/255760)
...
Threshold **4.8607** — 14/17 breaks fall below it (misses), 381/255760 controls fall at/above (false positives).
...
xychart-beta
    ...
    y-axis "recall %" 0 --> 110
    bar [33.3, 100.0, 100.0, 25.0, 100.0]
```
...
- **faker_js_error_flip_3** (`error_flip`) — score 4.0527, 0.8080 below threshold, reason: `none`
  - _Rationale:_ seed() override throws, breaking the determinism contract
- **faker_js_runtime_fetch_1** (`runtime_fetch`) — score 3.5478, 1.3130 below threshold, reason: `none`
  - _Rationale:_ Runtime fetch from a locale data file
- **faker_js_runtime_fetch_2** (`runtime_fetch`) — score 2.4487, 2.4120 below threshold, reason: `none`
  - _Rationale:_ ImageProvider.url() issues a live network fetch
- **faker_js_runtime_fetch_3** (`runtime_fetch`) — score 1.9712, 2.8895 below threshold, reason: `none`
...
| uncaught | 37.5% | 3/8 | Scorer currently misses — known gap |
