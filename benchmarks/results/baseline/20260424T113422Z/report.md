...
Generated: 2026-04-24T11:47:04.854068+00:00

## Headline

| Corpus | Lang | AUC | Recall | FP | Gap | N_fix | N_ctrl | Thr |
|:---|:---|---:|---:|---:|---:|---:|---:|---:|
| fastapi | python | 0.9880 | 71.3% | 0.8% | -4.371 | 32 | 79623 | 5.278 |
| rich | python | 0.9935 | 93.3% | 0.1% | -2.116 | 15 | 68598 | 4.164 |
| faker | python | 0.9530 | 73.3% | 0.7% | -4.946 | 15 | 75996 | 5.211 |
| hono | typescript | 0.8107 | 60.0% | 0.4% | -7.471 | 15 | 54717 | 4.277 |
| ink | typescript | 0.9888 | 86.7% | 0.4% | -4.633 | 15 | 16678 | 4.826 |
| faker-js | typescript | 0.9408 | 20.0% | 0.8% | -7.066 | 15 | 255760 | 4.773 |
...
- **Recall (mean across categories):** 71.3%
- **FP rate on real PR hunks:** 0.8% (246/79623)
...
xychart-beta
    ...
    y-axis "recall %" 0 --> 110
    bar [66.7, 100.0, 66.7, 66.7, 66.7, 100.0, 0.0, 100.0, 75.0]
```
...
| dependency_injection | 66.7% | 2/3 | 5.711 | 5.025 | 6.641 | dependency_injection_1, dependency_injection_2, dependency_injection_3 |
...
| hard | 0.0% | 0/6 | Stage 1.5 call-receiver catch — receiver novelty |
...
- **FP rate on real PR hunks:** 0.1% (76/68598)
...
xychart-beta
    ...
    y-axis "recall %" 0 --> 110
    bar [100.0, 100.0, 100.0, 66.7, 100.0]
```
...
| print_manual | 66.7% | 2/3 | 4.525 | 2.630 | 6.730 | print_manual_1, print_manual_2, print_manual_3 |
...
| hard | 50.0% | 1/2 | Stage 1.5 call-receiver catch — receiver novelty |
...
- **Recall (mean across categories):** 73.3%
- **FP rate on real PR hunks:** 0.7% (244/75996)
...
xychart-beta
    ...
    y-axis "recall %" 0 --> 110
    bar [66.7, 66.7, 100.0, 33.3, 100.0]
```
...
| Category | Recall | Hits | Mean break score | Min | Max | Fixtures |
|:---|---:|---:|---:|---:|---:|:---|
| mimesis_alt | 66.7% | 2/3 | 3.201 | 1.843 | 5.729 | mimesis_alt_1, mimesis_alt_2, mimesis_alt_3 |
| numpy_random | 66.7% | 2/3 | 5.228 | 4.593 | 5.786 | numpy_random_1, numpy_random_2, numpy_random_3 |
| requests_source | 100.0% | 3/3 | 7.073 | 6.596 | 7.380 | requests_source_1, requests_source_2, requests_source_3 |
| sqlalchemy_sink | 33.3% | 1/3 | 5.545 | 4.571 | 6.957 | sqlalchemy_sink_1, sqlalchemy_sink_2, sqlalchemy_sink_3 |
...
| ID | Category | BPE | Flagged | Reason | File | Lines | Rationale |
|:---|:---|---:|:---:|:---|:---|:---|:---|
| mimesis_alt_1 | mimesis_alt | 1.843 | ✓ | import | breaks/break_mimesis_alt_1.py | 19–46 | mimesis Person/Address/Finance with Gender enum — competing fake-data |
...
| mimesis_alt_3 | mimesis_alt | 2.032 | ✗ | none | breaks/break_mimesis_alt_3.py | 14–26 | Faker() calling rare financial provider methods (aba, bban, iban, swift, cryptocurrency_code) — no foreign import, BPE familiar, but thes… |
| numpy_random_1 | numpy_random | 4.593 | ✓ | import | breaks/break_numpy_random_1.py | 19–46 | numpy.random Generator/default_rng for name and age generation instead |
...
| numpy_random_3 | numpy_random | 5.304 | ✗ | none | breaks/break_numpy_random_3.py | 15–32 | numpy rng.choice and rng.shuffle called on a default_rng instance — receiver methods foreign to faker corpus. rng.integers is known but r… |
| requests_source_1 | requests_source | 7.380 | ✓ | import | breaks/break_requests_source_1.py | 17–47 | requests.get() fetching real user data from HTTP APIs instead of local |
...
| sqlalchemy_sink_1 | sqlalchemy_sink | 6.957 | ✓ | import | breaks/break_sqlalchemy_sink_1.py | 17–49 | SQLAlchemy ORM model + session.add/commit to persist fakes — DB-sink |
| sqlalchemy_sink_2 | sqlalchemy_sink | 4.571 | ✗ | none | breaks/break_sqlalchemy_sink_2.py | 16–28 | csv.DictWriter + io.StringIO export sink — csv and io imported outside hunk. Canonical: SQLAlchemy session at break sites; csv.DictWriter… |
| sqlalchemy_sink_3 | sqlalchemy_sink | 5.109 | ✗ | none | breaks/break_sqlalchemy_sink_3.py | 15–30 | SQLAlchemy Session.bulk_save_objects + Session.flush + Session.expire_all — these receiver methods on a Session object are foreign to fak… |
| threading_provider_1 | threading_provider | 7.339 | ✓ | import | breaks/break_threading_provider_1.py | 19–53 | threading.Thread + Lock + Event parallel fake-data generator class in |
...
- **numpy_random_3** (`numpy_random`) — score 5.3037, -0.0926 below threshold, reason: `none`
  - _Rationale:_ numpy rng.choice and rng.shuffle called on a default_rng instance — receiver methods foreign to faker corpus. rng.integers is known but rng.choice/rng.shuffle appear at 0 faker call sites. Single axis: rarely-called rng receiver methods.
- **sqlalchemy_sink_3** (`sqlalchemy_sink`) — score 5.1090, 0.1020 below threshold, reason: `none`
  - _Rationale:_ SQLAlchemy Session.bulk_save_objects + Session.flush + Session.expire_all — these receiver methods on a Session object are foreign to faker corpus (0 sites). Single axis: bulk_save_objects/flush/expire_all never called in faker internals.
- **sqlalchemy_sink_2** (`sqlalchemy_sink`) — score 4.5705, 0.6406 below threshold, reason: `none`
  - _Rationale:_ csv.DictWriter + io.StringIO export sink — csv and io imported outside hunk. Canonical: SQLAlchemy session at break sites; csv.DictWriter at 0 faker corpus sites.
- **mimesis_alt_3** (`mimesis_alt`) — score 2.0319, 3.1791 below threshold, reason: `none`
  - _Rationale:_ Faker() calling rare financial provider methods (aba, bban, iban, swift, cryptocurrency_code) — no foreign import, BPE familiar, but these methods appear at 0 call sites in the faker corpus itself. Single axis: rarely-called financial callee methods.
...
| Difficulty | Recall | Hits | Definition |
|:---|---:|---:|:---|
| medium | 80.0% | 4/5 | Stage 2 BPE catch — token-level novelty, no foreign import |
| hard | 40.0% | 2/5 | Stage 1.5 call-receiver catch — receiver novelty |
...
- **FP rate on real PR hunks:** 0.4% (26/16678)
...
- **FP rate on real PR hunks:** 0.8% (370/255760)
...
| hard | 0.0% | 0/2 | Stage 1.5 call-receiver catch — receiver novelty |
