...
// is absent from the repo at the pinned SHA (leveldb_options_create,
// leveldb_open, leveldb_writeoptions_create, leveldb_put, leveldb_close = 0
// hits tree-wide, no <leveldb/c.h>) — curl keeps connection metrics in its own
// progress struct (Curl_pgrsUpdate), never a foreign LSM key-value store. No
// foreign include is present in the hunk, so the catch rests entirely on the
// bare leveldb_* callee resolution.
