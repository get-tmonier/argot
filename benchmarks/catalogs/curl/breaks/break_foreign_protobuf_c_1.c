...
// length-prefixed wire buffer; protobuf-c is absent from the repo at the
// pinned SHA (protobuf-c/protobuf-c.h, protobuf_c_message_get_packed_size,
// protobuf_c_message_pack, protobuf_c_message_free_unpacked = 0 hits
// tree-wide) — curl frames HTTP/2 through nghttp2's own submit/callbacks,
// never a foreign schema serializer. Import masked by the quoted include, so
// the catch must come from the bare protobuf_c_* callee.
