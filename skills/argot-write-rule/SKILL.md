...
description: Codify a repo convention argot's built-ins don't cover into a scripted custom rule — a `.argot/rules/NAME/rule.toml` manifest plus a sandboxed Rhai script that fires exactly like a built-in, gated on a green fixture suite before it ever sees a real diff. Use when the user asks to "write an argot rule for X", "codify this convention", "ban this pattern with argot", or wants a repo-specific check beyond the twelve built-ins. Distinct from argot-check (running the existing rules) and argot-setup (fitting the voice model).
---
...
2. **Decide detectability honestly.** A convention is a shape tree-sitter can
   ...
   - **`read_repo_file(path)` / `repo_paths(glob)`** read the *rest of the
     ...
     other file's contents as a constant — a copied list goes stale silently,
     and the rule keeps passing while it does. These **do** work in
     `argot rules test`: repo access is rooted at the fixture case directory,
     so ship the sibling files next to `input.<ext>`.
