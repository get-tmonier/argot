...
//!
//! The tree-sitter captures are extracted with manual
//! `TreeCursor`/`child_by_field_name`/`children` traversal rather than the
//! Query API (matching the style of `python.rs`).
//!
...
//!   (not `import_statement`) and is NOT captured — the capture query
//!   `(import_statement source: (string) @src)` matches only `import_statement`.
...
//!   `root.has_error()`: that adapter's guard only returns empty on a parse
...
impl TypeScriptAdapter {
    ...
    /// trait — an inherent `extract_callees` method.
    ...
    /// (`import { z } from "zod"` → `(z, zod)`, `import * as np from "numpy"` →
    /// `(np, numpy)`), so the call-receiver gate can tell a hunk that *uses* a
    /// foreign dependency's symbol from the repo's own code. Relative imports
    /// are repo-internal — see `internal_import_bindings`. The module is the raw
    /// specifier, matching `extract_imports` so `is_foreign` lines up.
...
impl LanguageAdapter for TypeScriptAdapter {
    ...
    fn import_bindings(&self, source: &str) -> Vec<(String, String)> {
        TypeScriptAdapter::import_bindings(self, source)
    }
