...
//! `A3-F3-verdict.md`).
//!
//! No cargo features: this whole crate IS the `semantic` feature —
//! argot-core compiles it in only when built with `--features semantic` (an
//! optional dependency); with it absent from the build the base statistical
//! guardrail is byte-for-byte unchanged and pays no cost. This crate never
//! depends on argot-core, so it stays usable standalone and cannot cycle back.
