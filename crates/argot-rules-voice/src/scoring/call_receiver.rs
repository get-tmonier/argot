...
/// language-substrate crate — see `argot_lang::callees`'s module docs.
pub use argot_lang::callees::{callees_in_source_region, extract_callees, has_root_error};
use argot_lang::callees::{is_qualified, leading_namespace, non_none_callees};
...
impl CallReceiverScorer {
    ...
    pub fn export_model(&self, repo_root: &Path) -> CallReceiverModel {
        let rel = |p: &Path| {
            argot_engine::suppress::rel_string(p, repo_root)
