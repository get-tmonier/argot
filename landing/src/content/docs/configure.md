---
title: Configure
description: One committed argot.toml controls everything — what argot excludes ([exclude]), how it spots generated/data files ([detect]), and the hits you've accepted ([[mute]]) — plus inline comments and a gitignored argot.local.toml for personal overrides.
group: Guide
order: 5
---
...
defaults spelled out, so nothing is hidden. It has three sections:
...
gitignores it for you. It deep-merges over `argot.toml`: scalars there win, and
list entries (`paths`, `generated-markers`, `[[mute]]`) append.
