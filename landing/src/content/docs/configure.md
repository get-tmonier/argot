...
| `.argot/rules/<name>/` | `argot-write-rule` / by hand | Your [custom rules](/docs/custom-rules/) — `rule.toml` + `check.rhai`, authored not generated. | **Yes** — force-add past `.argot/.gitignore`; `uninstall` never deletes them. |
