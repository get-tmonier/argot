...
1. PROOF FIRST
   Run `argot audit` and show me the card. It needs no configuration and
   changes nothing. If it says the window touched no supported source, widen
   it with `--commits 200` or `--since 6m`. Keep the result — phase 6 uses it.

2. SCOPE
   Decide what should NOT shape the voice. Merge three sources into one list:
     - `argot init --suggest --format json` (generated / data-heavy dirs)
     - the fit-time note naming config files that are shaping the voice
...
4. VERIFY IT CATCHES SOMETHING
   In a real source file, add a throwaway import of a package this repo has
   never used plus a line using it. Run `argot check`, confirm it fires, then
   revert. If it does NOT fire and phase 1 also found nothing, the voice is
   diluted — go back to phase 2.
...
6. TUNE FROM MY OWN HISTORY
   Run `argot audit --format json` and read `over_firing` — rules tripping more
...
9. PROVE IT
   Offer me a repeatable local smoke check, and an A/B draft PR — a throwaway
   branch with one in-voice change and one deliberately foreign one, opened as
   a draft so I can see the annotations and comment on a real PR. Delete after.
