...
| Claude Code plugin: MCP | Plugin declares `argot mcp --repo .`; server exposes six consistently named, read-only tools over stdio: complete changeset checking, focused hunk checking and explanation, voice context, fit status, and conventions. Fitting remains an explicit local CLI/skill workflow. | Plugin enabled; executable on `PATH`; a fitted model for scoring/context tools. | Passive. | Server reports an error result when a fitted model is required but absent; it does not trigger itself or update `.argot/last-check.json`. | Current manifest; focused MCP unit and read-only check tests, 2026-07-30. | “The Claude plugin provides optional, agent-invoked MCP context and complete read-only changeset checks.” |
