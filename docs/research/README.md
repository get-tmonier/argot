...
| **Rarity axes exhausted, harness reborn** | 6 phases | Era 14 ran six pre-registered phases at the ≤1.0%-FP / 108-recall-floor gates on a rebuilt Rust bench harness (`crates/argot-bench`, reproduces the era-13.5 Python baseline exactly). Phase A (rarity weighting): REFUTED — corpus document frequency does not separate break callees from FP callees; foreign-paradigm callees are rare in-corpus by construction (faker-js: 18/21 fixture cluster events at df≤3, same band as the FP events); every formula destroyed cluster-branch catches. Phase B (diff-hunk calibration): REFUTED — real diff fragments are high-BPE-surprise, thresholds explode 3.8–5.3 → 8.8–11.9, recall collapses to 54.8%. Phase C (negative-shape primitive, 3 maths): REFUTED — all fire on controls, none reaches the targets. Phase D (parse-error host fallback): fastapi 32/32 (first 100% corpus) but FP-couples with the rare rule (faker-js 1.7% → 4.9%); shipped gated default-off. Phase E: era-13.5 auto-detect productionised into `argot calibrate` (the Rust port had silently pinned prod at pre-13.5 baseline). Phase F: CSF TypeScript boundary fixed. Scoring defaults unchanged at **108/115 (93.9%)**; per-callee frequency is exhausted as a mechanism family. | [evidence/era14-final.md](evidence/era14-final.md) |
| **The production path** | 3 root causes + 3 new mechanism families | Era 15 closed the gap between the bench and the shipped tool. Three measured root causes: check rebuilt attestation from the live tree so new code attested itself (#79 — the bug had been acting as the FP suppressor); check passed `file_path=None` (no cluster routing); calibration earned bonuses the check path couldn't. Shipped: scorer-config **v3 model artifact** (fit-time BPE stats + callee attestation + clusters + convention frequencies; deterministic, hashed), check-time cluster routing (fitted files only), row-granular data scope (replaces two file-level vetoes), parse-error host fallback ON (git-shaped fragments are the norm), **convention-rarity stage** (AST node-kind + identifier-morphology surprisal, bars = all-candidate max), **neighbourhood attestation** (definitions/imports/value bindings attest bare calls; corpus-known method segments don't alpha-fire), `this.`-rooted TS callees, error-tolerant python import extraction, n_cal aligned to the validated 100. A **production-path bench mode** (fixture planted on disk → real `fit` → `check --staged`) became the headline: libraries **111/115 (96.5%)**, applications **49/56 (87.5%)**, production FP ≈ 0 on 7/10 corpora; five corpora at 100% including fastapi 32/32. Moneta gauntlet: **10/12** subtle no-import breaks flagged (era start: 0 crossable). Refuted: sub-max thresholding, sustained token-surprise aggregation, nesting-depth conventions (all measured inside the calibration body). | [evidence/era15-production-path.md](evidence/era15-production-path.md) |
