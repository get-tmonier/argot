...
**Current phase**: Phase 7.4 — frozen CodeRankEmbed + best density head
...
- [x] 7.2 — density heads on BPE: kNN, GMM ([`phase-7/17-density-heads.md`](phase-7/17-density-heads.md)). Decision gate NOT cleared: best `synthetic_auc_mean` = 0.581 (gmm-8 medium-py) vs 0.85 target. Predictor was not the bottleneck — same plateau as 7.1. `case_swap` detectable by GMM at medium-ts (0.773–0.780); other mutations invisible. Best head for 7.4: gmm-8.
- [x] 7.3 — frozen CodeRankEmbed + current head ([`phase-7/18-pretrained-jepa.md`](phase-7/18-pretrained-jepa.md)). **Decision gate NOT cleared** (pilot only — small-py/small-ts × 3 seeds × size=3000). Best `synthetic_auc_mean` = 0.517 (small-py s2) vs 0.85 target; std ≤ 0.003. Strong gains on `shuffled_auc` (0.82–0.89), `cross_auc_same_lang` (0.75–0.76), `injected_auc` (0.94–0.97) — pretrained embeddings capture style/origin. Zero gain on mutation detection: same 0.48–0.52 plateau as 7.1/7.2. Conclusion: representation is not the bottleneck — the training objective is (both JEPA surprise and density score measure distance from home distribution; mutations stay inside it).
...
- 2026-04-19 — Phase 7.3 scoped down to small-bucket pilot (2 × 3 seeds, size=3000) instead of full 6-bucket grid. Reasons: encode-phase dominates runtime on MPS (2× slower than training), and 7.1/7.2 showed `synthetic_auc_mean` flat across all 3 sizes — a size-scaling run only worth doing if the pilot signal were ≥ 0.70. Observed 0.50–0.52 with std ≤ 0.003 makes the gate mathematically out of reach regardless of bucket.
- 2026-04-19 — `score_records` batched at size 128 (was per-record). Prior loop caused multi-minute silent hangs post-training on 768-dim × 2k held-out records.
- 2026-04-19 — benchmark feedback loop: explicit per-phase stdout lines ("score.good — held-out real edits — N recs in Ts"), per-phase timings, ETA in train epochs, compact run summary. Applies to all encoders, no per-call log spam.
...
- **2026-04-19**: Phase 7.2 complete. 4 density heads (knn-20, gmm-8, gmm-16, gmm-32) × 6 buckets × 3 seeds on BPE embeddings. **Decision gate NOT cleared**: best `synthetic_auc_mean` = 0.581 (gmm-8 medium-py) vs 0.85 target. Key finding: swapping JEPA predictor for density head does not help — from-scratch BPE embeddings are the bottleneck. Positive signals: `cross_auc_same_lang` improves to 0.65–0.87 (vs 0.46–0.65 in 7.1); `case_swap` detectable by GMM at medium-ts (0.773–0.780); `shuffled_auc` drops to 0.500 (density head has no sequential signal). `error_flip`/`quote_flip` remain at 0.500 (mutation no-ops). Best head for 7.4: gmm-8. Full write-up: `phase-7/17-density-heads.md`. Next: Phase 7.3 — frozen CodeRankEmbed.
...
- **2026-04-19**: Phase 7.3 pilot complete. Frozen CodeRankEmbed (768-dim, sentence-transformers, `trust_remote_code=True`, `max_seq_length=512`) + JEPA predictor on small-py / small-ts × 3 seeds × size=3000 × 50 epochs on MPS. **Decision gate NOT cleared**: best `synthetic_auc_mean` = 0.517 (small-py s2) vs 0.85 target, std ≤ 0.003 (no variance to close the gap). Scoped to pilot because 7.1/7.2 showed flat `synthetic_auc_mean` across all sizes — escalation to medium/large only justified if small showed signal. Secondary metrics jumped vs from-scratch: `shuffled_auc` 0.82–0.89, `cross_auc_same_lang` 0.75–0.76, `injected_auc` 0.94–0.97. Key infra fixes during runs: (a) `max_seq_length` cap on MPS to avoid OOM, (b) `score_records` batched to 128 to eliminate silent post-train hangs, (c) per-phase feedback loop with explanations + timings for long runs (`encode  done in 76s  (32 texts/s)` / `score.injected   done in 24s`). Full write-up: `phase-7/18-pretrained-jepa.md`. Decision: representation is not the bottleneck — the training objective is. Next: Phase 7.4 — CodeRankEmbed + gmm-8 density head, or pivot to mutation-aware supervision if that also fails.
