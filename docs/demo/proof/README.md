...
identity and timestamps. It stages the authored Django-style change, records
the resulting check JSON, commits that same change, then records the one-commit
audit in JSON, Markdown, and HTML. It requires exactly `argot 0.2.89`; this
...
integrity are not compiled into that build. This is an explicit semantic-off
record, not a claim about a released full-featured binary.
...
`checksums.sha256` covers the deterministic JSON, Markdown, and HTML receipts.
The check script rebuilds them in a temporary directory and compares both every
artifact checksum and the checksum manifest. Fixed timestamps, Git metadata,
fixture contents, and the exact Argot version make those receipts byte-for-byte
reproducible.
...
token sequence in the introduced Django-style import. Semantic, architecture,
and integrity are shown as unavailable in this development build.”
