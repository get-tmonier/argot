...
_IMPORT_QUERY_TS = _TS_LANGUAGE.query("(import_statement source: (string) @src)")
_IMPORT_QUERY_TSX = _TSX_LANGUAGE.query("(import_statement source: (string) @src)")
    ...
    for child in root.children:
        ...
        if child.type in ("lexical_declaration", "variable_declaration"):
            ...
            for sub in child.children:
                ...
                inner_sub = sub
                if sub.type == "as_expression":
                    inner_sub = sub.children[0] if sub.children else sub
                if inner_sub.type in _TS_DATA_LITERAL_TYPES:
                    # export default [...] or export default {...}
                    if _is_ts_value_literal_dominant(inner_sub):
...
class TypeScriptAdapter:
    ...
    def _get_parser_and_queries(self, extension: str) -> tuple[TsParser, object, object]:
        if extension == ".tsx":
