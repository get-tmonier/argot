# engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix5_2026_04_22.py
"""Phase 14 Exp #7 Step 8 — Score PR hunks with fix5 (per-PR recalibration).

Reads:  real_pr_base_rate_prs_with_sha_2026_04_22.jsonl   (cached, with mergeCommit SHAs)
Writes: real_pr_base_rate_hunks_fix5_2026_04_22.jsonl

Per-PR calibration: for each PR, rewind to pre-PR state (merge_sha^1), extract via
git archive, calibrate a fresh SequentialImportBpeScorer against that snapshot.

This removes temporal calibration mismatch by design: each PR is scored against
the repo as it existed just before the PR was merged.

Scoring uses fix4's merge-commit extraction verbatim (git show <merge_sha>:<path>).
Tokenizer is loaded once and shared across all 50 per-PR calibration runs.

Usage:
    uv run python engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix5_2026_04_22.py
    uv run python engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix5_2026_04_22.py --sanity-check
"""
...
_HUNKS_JSONL = _SCRIPT_DIR / "real_pr_base_rate_hunks_fix5_treesitter_2026_04_22.jsonl"
