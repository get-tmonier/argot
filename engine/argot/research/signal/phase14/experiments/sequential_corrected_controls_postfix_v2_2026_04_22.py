# engine/argot/research/signal/phase14/experiments/sequential_corrected_controls_postfix_v2_2026_04_22.py
"""Phase 14 Experiment 2c — Postfix V2: break fixtures scored with hunk-only + file_source (2026-04-22).

Change from exp #2c:
  Exp #2c scored FastAPI break fixtures via _extract_file_to_hunk_end (file-start-to-hunk-end),
  which fed a large prefix into both Stage 1 AND Stage 2 of the scorer.

  With the new scorer signature (Step 3):
    score_hunk(hunk_content, *, file_source=None)
    Stage 1: extract_imports(file_source) + hunk_content   (if file_source provided)
    Stage 2: hunk_content only                             (always)

  This postfix re-scores all break fixtures using the new calling convention:
    - hunk_content = _extract_hunk(path, start_line, end_line)  (just the hunk lines)
    - file_source  = full file text                             (for Stage 1 import context)

  ctrl_hunks (from sample_hunks_disjoint) are raw function/class bodies — passed as
  hunk_content with no file_source (they have no associated full-file context).

Hypothesis (same pre-registered criteria):
...
Hard gate: if synthetic recall drops on ANY domain, STOP and report BLOCKED.

Usage:
    uv run python \\
        engine/argot/research/signal/phase14/experiments/\\
        sequential_corrected_controls_postfix_v2_2026_04_22.py
"""
...
_SCORES_OUT = _SCRIPT_DIR / "sequential_corrected_controls_postfix_v2_2026_04_22_scores.json"
_DOCS_OUT = (
    _PROJECT_ROOT
    / "docs"
    / "research"
    ...
    / "phase14"
    / "experiments"
    / "sequential_corrected_controls_postfix_v2_2026-04-22.md"
)
...
# Exp #2c scores for comparison
_EXP2C_SCORES_PATH = _SCRIPT_DIR / "sequential_corrected_controls_2026_04_22_scores.json"
...
# ---------------------------------------------------------------------------
# Per-seed runner helpers — NEW calling convention
# ---------------------------------------------------------------------------
...
def _score_fixtures_fastapi(
    ...
    """FastAPI: hunk_content = hunk lines only; file_source = full file for Stage 1."""
    results: list[dict[str, Any]] = []
    for f in fixtures:
        path = _FASTAPI_DIR / f["file"]
        hunk = _extract_hunk(path, f["hunk_start_line"], f["hunk_end_line"])
        file_source = path.read_text(encoding="utf-8", errors="replace")
        scored = scorer.score_hunk(hunk, file_source=file_source)
        results.append(
            {
                "name": f["name"],
...
def _score_fixtures_rich(
    ...
    """Rich: hunk_content = hunk lines only; file_source = full file for Stage 1."""
    results: list[dict[str, Any]] = []
    for f in fixtures:
        path = _RICH_DIR / f["file"]
        hunk = _extract_hunk(path, f["hunk_start_line"], f["hunk_end_line"])
        file_source = path.read_text(encoding="utf-8", errors="replace")
        scored = scorer.score_hunk(hunk, file_source=file_source)
        results.append(
...
def _score_ctrl_hunks(
    ...
    """ctrl_hunks: raw function bodies, no file_source."""
    results: list[dict[str, Any]] = []
...
def _run_faker_corrected() -> dict[str, Any]:
    ...
    Break fixtures scored with hunk_content + file_source (new calling convention).
    """
...
def _write_report(
    ...
    fk_verdict = (
        "VALIDATED"
        if faker_data["recall"] == 1.0 and fk_fp_class == "VALIDATED"
        else (
            "REJECTED"
            if faker_data["recall"] < 1.0 or faker_data["fp_rate"] > 0.20
            else "ZONE_GRISE"
        )
    )
    ...
    # Exp #2c comparison data
    exp2c_fa_thresholds = [s["bpe_threshold"] for s in exp2c_scores["fastapi"]["seeds"]]
    exp2c_ri_thresholds = [s["bpe_threshold"] for s in exp2c_scores["rich"]["seeds"]]
    exp2c_fa_mean_t = statistics.mean(exp2c_fa_thresholds)
    exp2c_ri_mean_t = statistics.mean(exp2c_ri_thresholds)
    exp2c_fk_t = exp2c_scores["faker"]["bpe_threshold"]
    exp2c_fa_fp = [s["fp_rate"] for s in exp2c_scores["fastapi"]["seeds"]]
    exp2c_ri_fp = [s["fp_rate"] for s in exp2c_scores["rich"]["seeds"]]
    exp2c_fa_mean_fp = statistics.mean(exp2c_fa_fp)
    exp2c_ri_mean_fp = statistics.mean(exp2c_ri_fp)
    exp2c_fk_fp = exp2c_scores["faker"]["fp_rate"]

    lines: list[str] = [
        "# Phase 14 Experiment 2c Postfix V2 — Break fixtures scored hunk-only + file_source (2026-04-22)",  # noqa: E501
        "",
        "**Scorer:** `SequentialImportBpeScorer` (Step 3 — new `score_hunk(hunk_content, *, file_source)` signature)",  # noqa: E501
        "",
    ...
    lines += [
        ...
        "|---|---|---|---|",
        f"| FastAPI | {exp2c_fa_mean_t:.4f} | {fa_mean_t:.4f} | {fa_mean_t - exp2c_fa_mean_t:+.4f} |",  # noqa: E501
        f"| rich | {exp2c_ri_mean_t:.4f} | {ri_mean_t:.4f} | {ri_mean_t - exp2c_ri_mean_t:+.4f} |",  # noqa: E501
        f"| faker | {exp2c_fk_t:.4f} | {faker_data['bpe_threshold']:.4f} | {faker_data['bpe_threshold'] - exp2c_fk_t:+.4f} |",  # noqa: E501
        "",
        "| domain | exp #2c FP rate (mean) | postfix v2 FP rate (mean) | delta_fp |",
        "|---|---|---|---|",
        f"| FastAPI | {exp2c_fa_mean_fp:.0%} | {fa_mean_fp:.0%} | {fa_mean_fp - exp2c_fa_mean_fp:+.0%} |",  # noqa: E501
        f"| rich | {exp2c_ri_mean_fp:.0%} | {ri_mean_fp:.0%} | {ri_mean_fp - exp2c_ri_mean_fp:+.0%} |",  # noqa: E501
        f"| faker | {exp2c_fk_fp:.0%} | {faker_data['fp_rate']:.0%} | {faker_data['fp_rate'] - exp2c_fk_fp:+.0%} |",  # noqa: E501
        "",
        "---",
        "",
    ...
    for name in ri_break_names:
        margins_r: list[float] = []
        flagged_count_r = 0
        cat_r = ""
        for s in ri_seeds:
            r = next((x for x in s["fixtures"] if x["name"] == name), None)
            if r:
                margins_r.append(r["bpe_score"] - s["bpe_threshold"])
                if r["flagged"]:
                    flagged_count_r += 1
                cat_r = r["category"]
        if margins_r:
            min_m = min(margins_r)
            always = (
                "YES" if flagged_count_r == N_SEEDS else f"NO ({flagged_count_r}/{N_SEEDS})"
            )
            lines.append(f"| {name} | {cat_r} | {min_m:+.4f} | {always} |")
    ...
    if overall == "VALIDATED":
        ...
        rejected = [
            d
            for d, v in [("FastAPI", fa_verdict), ("rich", ri_verdict), ("faker", fk_verdict)]
            if v == "REJECTED"
        ]
        lines += [
...
def main() -> None:
    print("Phase 14 Experiment 2c Postfix V2 — hunk-only + file_source calling convention", flush=True)
    ...
    print("\nLoading exp #2c scores for comparison...", flush=True)
    exp2c_scores: dict[str, Any] = json.loads(_EXP2C_SCORES_PATH.read_text(encoding="utf-8"))
    ...
    _write_report(_DOCS_OUT, fastapi_data, rich_data, faker_data, exp2c_scores)
