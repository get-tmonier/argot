# engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix9_2026_04_22.py
"""Phase 14 Exp #7 Step P — Score PR hunks with fix9 (model A data-dominant exclusion).

Reads:  real_pr_base_rate_prs_with_sha_2026_04_22.jsonl   (cached, with mergeCommit SHAs)
Writes: real_pr_base_rate_hunks_fix9_2026_04_22.jsonl

fix9 change over fix8: data-dominant files are now excluded from model A's *training*
corpus (not just the calibration sampling pool).  The SequentialImportBpeScorer default
is exclude_data_dominant=True — no explicit kwarg needed here.  FastAPI has zero
data-dominant files so this run is expected to be a no-op regression check vs fix8.

Usage:
    uv run python engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix9_2026_04_22.py
    uv run python engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix9_2026_04_22.py --sanity-check
"""
...
_HUNKS_JSONL = _SCRIPT_DIR / "real_pr_base_rate_hunks_fix9_2026_04_22.jsonl"
