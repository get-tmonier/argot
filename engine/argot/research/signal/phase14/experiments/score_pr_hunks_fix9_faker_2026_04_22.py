# engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix9_faker_2026_04_22.py
"""Phase 14 Prompt S — Score faker PR hunks with fix9 (model A data-dominant exclusion).

Reads:  faker_real_pr_base_rate_prs_2026_04_22.jsonl
Writes: real_pr_base_rate_hunks_fix9_faker_2026_04_22.jsonl

fix9 change over fix8: data-dominant files (faker locale string tables) are now excluded
from model A's *training* corpus (not just the calibration sampling pool).  Expected:
model A becomes code-fluent, normal faker code scores drop, and the calibration floor
falls from ~7.15 toward ~4-5, lifting Stage 2 fixture recall from 38% toward ≥80%.

Usage:
    uv run python engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix9_faker_2026_04_22.py
    uv run python engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix9_faker_2026_04_22.py --sanity-check
"""
...
_HUNKS_JSONL = _SCRIPT_DIR / "real_pr_base_rate_hunks_fix9_faker_2026_04_22.jsonl"

_FAKER_REPO = _REPOS_DIR / "faker"
_REPO_GH = "joke2k/faker"
_N_CAL = 250  # data-dominant filter reduces pool to ~275; N=250 is safe max
_CAL_SEED = 0
...
def main(sanity_check: bool = False) -> None:
    ...
    for i, pr in enumerate(prs):
        ...
        n_autogen = sum(1 for r in src_records if r.get("reason") == "auto_generated")

        print(
            f"  [{i + 1:2d}/{len(prs)}] PR #{pr_num}  {pr['mergedAt'][:10]}  "
            f"pre={pre_sha[:7]}  cal_thr={cal_threshold:.4f}  "
            f"src={n_src_hunks}  flagged={n_flagged}  st1={n_st1}  st2={n_st2}  autogen={n_autogen}",
            flush=True,
        )
    ...
    src_autogen = sum(
        1 for r in all_records if not r["is_test"] and r.get("reason") == "auto_generated"
    )
