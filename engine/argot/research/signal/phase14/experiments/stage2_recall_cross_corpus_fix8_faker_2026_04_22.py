# engine/argot/research/signal/phase14/experiments/stage2_recall_cross_corpus_2026_04_22.py
"""Phase 14 Experiment — Stage 2 Recall Cross-Corpus Probe.

Hypothesis: Faker's Stage 2 blindness is purely threshold-driven, not content-driven.
Fixture BPE scores should be comparable across FastAPI / rich / faker hosts; only
the calibration threshold differs.

Design:
    Stage2OnlyScorer (Stage 1 permanently disabled) is calibrated on each host
    corpus's pre-merge snapshot.  All 8 stage2_only fixtures are injected into
    4-5 clean host PRs per corpus.

    FastAPI baseline (N=100 per host): reused from Step O — not re-run.

    Rich (N=200 per host, 5 host PRs):
        #4079, #4077, #4076, #4075, #3941  — all clean in fix7 base-rate run.

    Faker (N=500 per host, 5 host PRs):
        #2352, #2351, #2350, #2349, #2348  — all clean in fix7 base-rate run.

    For each (host, fixture) pair:
        - raw BPE score
        - max threshold
        - p99/p95 counterfactual thresholds
        - margin = bpe_score - threshold
        - flagged (bool)
...
# ---------------------------------------------------------------------------
# Stage2OnlyScorer (same as in stage2_recall_probe_2026_04_22.py)
# ---------------------------------------------------------------------------
...
_DOCS_OUT = (
    ...
    / "phase14"
    / "experiments"
    / "stage2_recall_cross_corpus_fix8_faker_2026-04-22.md"
)
...
def _write_report(
    ...
    all_corpora = [
        ("fastapi", None),  # baseline — no HostResult objects
        ("rich", rich_results),
        ("faker", faker_results),
    ]

    # Build per-fixture median BPE scores per corpus
    ...
    # FastAPI fixture BPE scores (from Step O report)
    ...
    lines: list[str] = [
        ...
        " Feasible ceiling per N=500 revalidation |",
        f"| Faker | {_FAKER_N_CAL} | {', '.join(f'#{p}' for p in _FAKER_HOST_PR_NUMS)} |"
        " Matches faker experiment config |",
        "",
        "---",
        "",
    ...
    # Build §0 matrix: fixture × (FastAPI, rich, faker)
    header = "| fixture |"
    ...
    lines.append("A large deviation would mean corpus tokenization diverges — invalidating the")
    lines.append("hypothesis that threshold is the sole differentiator.")
    lines += [
        "",
    ...
    # FastAPI per-host thresholds from Step O
    ...
    if faker_results:
        ...
        lines.append(
            ...
            if f_total
            else f"| Faker | 0/0 | 0 | N/A |"
        )

    lines += [""]

    # Detailed per-host tables for rich and faker
    ...
    lines.append("from max to p99/p95 unblock faker recall?")
    lines += [
        "",
    ...
    # Build verdict
    has_rich = bool(rich_results)
    has_faker = bool(faker_results)
    ...
    if has_faker:
        ...
        if max_gap <= 0.5:
            score_verdict = (
                "Score homogeneity CONFIRMED: fixture BPE scores are within ±0.5 across all hosts."
            )
        else:
            score_verdict = (
                f"Score homogeneity PARTIAL: max gap is {max_gap:.3f} — "
                "some fixtures diverge across corpora."
            )

        if faker_max_catch == 0.0 and faker_p99_catch == 0.0:
            threshold_verdict = (
                f"**Faker Stage 2 is NOT rescuable by threshold alone.** "
                f"Even at p99 (median ≈{f_med_p99:.2f}) the catch rate is 0%. "
                f"Faker BPE scores are structurally below the calibration floor. "
                f"This is a content/corpus mismatch beyond threshold tuning."
            )
        elif faker_max_catch == 0.0 and faker_p99_catch > 0.0:
            threshold_verdict = (
                f"**Hypothesis CONFIRMED (partially):** Faker Stage 2 blindness is "
                f"threshold-driven. At max (≈{f_med_max:.2f}), catch rate is 0%; "
                f"at p99 (≈{f_med_p99:.2f}), catch rate rises to {faker_p99_catch:.0%}. "
                f"p99 is a viable unified threshold strategy if FP rate stays acceptable."
            )
        elif faker_max_catch > 0.0:
            threshold_verdict = (
                f"**Hypothesis CONFIRMED:** Faker Stage 2 fires at max threshold "
                f"({faker_max_catch:.0%} catch rate). "
                f"The faker blindness observed in the N=500 base-rate run may have been "
                f"threshold calibration instability, not a structural limit."
            )
        else:
            threshold_verdict = "**Inconclusive** — insufficient results to determine hypothesis."
    else:
        score_verdict = "Faker results not available."
        threshold_verdict = "Faker results not available."
    ...
    if has_rich:
        r_flagged_max, r_total = _catch_rate(rich_results, "max")
        r_catch = r_flagged_max / r_total if r_total else 0.0
        if r_catch >= 0.875:  # ≥7/8
            rich_verdict = (
                f"**Rich recall: {r_catch:.0%}** — Rich behaves comparably to FastAPI. "
                f"The ~4.1-4.4 threshold regime does not create blind spots for these fixtures."
            )
        elif r_catch >= 0.5:
            rich_verdict = (
                f"**Rich recall: {r_catch:.0%}** — Rich has partial blind spots at max threshold. "
                f"Some fixtures clear the ~4.1-4.4 regime, others don't."
            )
        else:
            rich_verdict = (
                f"**Rich recall: {r_catch:.0%}** — Rich has unexpected Stage 2 blind spots. "
                f"The threshold regime (~4.1-4.4) is significantly higher than FastAPI (~4.0). "
                f"Investigate calibration distribution."
            )
        lines += [rich_verdict, ""]
    ...
    if has_faker:
        lines += [
            f"| Are fixture BPE scores homogeneous across hosts? |"
            f" {'Yes (±0.5)' if max_gap <= 0.5 else f'Partial (max gap {max_gap:.2f})'} |",
            f"| Is faker blindness purely threshold-driven? |"
            f" {'Yes — p99 rescues it' if faker_max_catch == 0.0 and faker_p99_catch > 0.0 else 'No — even p99 fails' if faker_p99_catch == 0.0 else 'Yes — even max catches fixtures'} |",
            f"| Does p99 unblock faker without FP explosion? |"
            f" See §4 — requires FP base-rate validation before adopting |",
        ]

    if has_rich:
        r_flagged_max_count, r_total_count = _catch_rate(rich_results, "max")
        lines.append(
            f"| Rich recall at max threshold? |"
            f" {r_flagged_max_count}/{r_total_count} = {r_flagged_max_count / r_total_count:.0%} |"
            if r_total_count
            else "| Rich recall at max threshold? | N/A |"
        )
...
def main() -> None:
    print("Phase 14 Stage-2 Recall Cross-Corpus Probe", flush=True)
