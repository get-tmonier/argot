# engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix3_2026_04_22.py
"""Phase 14 Exp #7 Step 5 — Score PR hunks with fix3 (extraction decoupled).

Reads:  real_pr_base_rate_prs_2026_04_22.jsonl   (cached — NOT re-mined)
Writes: real_pr_base_rate_hunks_fix3_2026_04_22.jsonl

Calibration: seed 0, n=100 hunks from .argot/research/repos/fastapi source.
Scoring:     SequentialImportBpeScorer with fix3 applied:
             - Stage 1 uses file_source to extract imports from full file context
               plus any imports in the hunk itself (extraction decoupled)
             - Stage 2 scores hunk_content only (not file-start-to-hunk-end)
Extraction:  hunk_content = just the hunk lines from current HEAD
             file_source  = full file text from current HEAD

Usage:
    uv run python engine/argot/research/signal/phase14/experiments/score_pr_hunks_fix3_2026_04_22.py
"""
    ...
    for i, pr in enumerate(prs):
        ...
        def _score_hunks(
            ...
            for hi, hunk in enumerate(hunks):
                ...
                # fix3: pass hunk_content (just hunk lines) + file_source (full file)
                file_text = fp.read_text(encoding="utf-8", errors="replace")
                hunk_content = _extract_hunk(fp, hunk["start_line"], hunk["end_line"])
